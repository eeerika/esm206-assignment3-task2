...
- The third piece doesn't show up
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create a new column called 'make_model'
...
```{r}
# Your code here: 
gasol_hookshots <- lakers %>% # Start from lakers dataset
  filter(player == "Pau Gasol", type == "hook") %>% # Filter for Pau Gasol and hook
  mutate(date_new = ymd(date)) %>% # New column with "Date" class using `ymd()`
  mutate(month = month(date_new)) %>% # New column with month, from date_new
  count(month) # Indicates number of Gasol's hook shots recorded by month
...
storms_danger <- storms %>% # Start from storms data
  mutate(danger = case_when( # Add a new column danger, which contains... 
    ...
    TRUE ~ "moderate" # "moderate" if status is anything other than hurricane
  )
  )
